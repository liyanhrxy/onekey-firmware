...
int get_base32_encode_address(uint8_t *in, char *out, size_t out_len, uint32_t network_id) {
    ...
    switch (network_id) {
        ...
        case CHAIN_ID_TESTNET: {
            if (out_len < (size_t)(8 + 34 + 8 + 1)) { return CFXADDR_ERROR_WRONG_LENGTH; };
        ...
        default: {
            ...
            if (out_len < (size_t)(length + 34 + 8 + 1)) { return CFXADDR_ERROR_WRONG_LENGTH; };
    ...
    for (size_t ii = 0; ii < 8; ++ii) {
        out[prefix_len + 34 + ii] = CHARSET[(uint8_t)(checksum >> ((7 - ii) * 5) & 0x1f)];
    }
...
static void layoutConfluxConfirmTx(uint8_t *to, uint32_t to_len,
                                    const uint8_t *value, uint32_t value_len, const TokenType *token, uint32_t chain_ids) {
  bignum256 val = {0};
...
static bool conflux_signing_check(const ConfluxSignTx *msg) {
  ...
  if (msg->gas_price.size + msg->gas_limit.size + msg->storage_limit.size > 30) {
...
void conflux_signing_init(ConfluxSignTx *msg, const HDNode *node) {
  ...
  rlp_length += rlp_calculate_length(msg->storage_limit.size, msg->storage_limit.bytes[0]);
  rlp_length += rlp_calculate_length(msg->epoch_height.size,
                                     msg->epoch_height.bytes[0]);
  rlp_length +=
...
void conflux_message_sign_cip23(const ConfluxSignMessageCIP23 *msg,
  ...
  sha3_256_Init(&ctx);
  sha3_Update(&ctx, (const uint8_t *)"\x19\x01", 2);  // CIP23 magic is the same with EIP712
